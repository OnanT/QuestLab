/********************************************************************
 *  add_ant_grasshopper_lesson.sql
 *
 *  Inserts a complete lesson (topic → concept → lesson → quizzes)
 *  for the “The Ant and the Grasshopper – Future Tense Verbs”.
 *
 *  Run it with:
 *      docker exec -i questlab_postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" < add_ant_grasshopper_lesson.sql
 *
 *  -----------------------------------------------------------------
 *  STEP 0 –  Set the teacher (owner) of the lesson.
 *  -----------------------------------------------------------------
 */
\set teacher_id 2   -- <-- CHANGE THIS TO THE ID OF YOUR TEACHER USER


/********************************************************************
 *  STEP 1 –  Pick an existing curriculum_subject & term.
 *
 *  For this example we use:
 *      • Country   = Nevis (id = 1)          – already in the seed data
 *      • Subject   = Language Arts (id = 4)  – already in the seed data
 *      • Grade     = 3‑4 (grade_level = 3)   – matches the lesson’s grade level
 *      • School year = 2025‑2026 (school_year_id = 1)
 *      • Term      = Term 1 (term_id = 1)
 *
 *  If you need a different subject/term, look them up first:
 *      SELECT * FROM curriculum_subjects WHERE country_id = 1 AND subject_id = 4;
 *      SELECT * FROM terms WHERE school_year_id = 1;
 ********************************************************************/
-- curriculum_subject_id for Nevis + Language Arts (grade 3)
-- In the seed data this row has id = 4
\set curriculum_subject_id 4

-- term_id for Nevis 2025‑2026 Term 1
\set term_id 1
...
INSERT INTO public.lessons (
    concept_id, title, content_html, creator_id,
...
$$,
    ...
    'Language Arts',                 -- category (fits the story‑telling focus)
    'beginner',                      -- difficulty
    40,                              -- estimated_time (minutes)
    50,                              -- points awarded for completing the lesson
    '3-4',                           -- grade_levels (stored as text)
 ...
 *
 *  Four quizzes, one per future‑tense structure covered:
 *      1. Identify a future‑tense verb (“will”)
 *      2. Choose the correct base verb after “will”
 *      3. Identify a “going to” construction
 *      4. Choose the correct form of “going to” for a subject
 ********************************************************************/
...
/********************************************************************
 *  STEP 6 –  Reset the sequences so future INSERTs start at the
 *           next unused id (important after manual id insertion).
 ********************************************************************/
...
/********************************************************************
 *  DONE!  The lesson is now available via the API:
 *      GET /lessons/<lesson_id>
 *      GET /quizzes?lesson_id=<lesson_id>
 *
 *  You can also view it in the front‑end under the “Language Arts”
 *  section for Grade 3‑4.
 ********************************************************************/